...
start_process (void *file_name_)
{
  ...
  //  thread_current()->parent->c_load_success=true;
...
sema_up(&t->finished_flag);
 ...
 if (!success) {
   ...
   // thread_current()->parent->c_load_success=false;
    //sema_up(&thread_current()->parent->c_lock);
    thread_exit();
  }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
 ...
 // acquire_filesys_lock();
  ...
  char * save_ptr;
  fn_cp = strtok_r(fn_cp,DELIMETERS,&save_ptr);

  file = filesys_open (fn_cp);
  thread_current()->prg_file = file;
...
//////////////////////////////////////////////////////////////////////////////////
 ...
 argVars[0] = strtok_r(copy, DELIMETERS, &save_ptr);
 while(true) {
	argVars[argCounter] = strtok_r(NULL, DELIMETERS, &save_ptr);
	if(argVars[argCounter] == NULL)
		break;    
	argCounter++;   
}
  ...
  int *address = calloc(argCounter,sizeof(int));
  for (int i = argCounter -1; i >= 0; i--) {
      *esp -= strlen(argVars[i]) + 1;
      memcpy(*esp,argVars[i],strlen(argVars[i]) + 1);
  ...
  *esp-=sizeof(int);
  ...
  for(i=argCounter-1;i>=0;i--) {
    *esp-=sizeof(int);
    memcpy(*esp,&address[i],sizeof(int));
  }

  int startArgs = *esp;
  *esp-=sizeof(int);
  memcpy(*esp,&startArgs,sizeof(int));

  *esp-=sizeof(int);
  memcpy(*esp,&argCounter,sizeof(int));
